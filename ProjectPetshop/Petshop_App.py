...
def login():
    ...
    try:
        with open('data/login.csv', 'r') as file:
            ...
            for row in reader:
                if len(row) == 4:
                    a, b, c, d = row  # username, password, email, pin
                    if username == a and password == b:
                        sukses = True
                        pin_required = True if d else False
                        break
                elif len(row) == 3:
                    a, b, c = row  # username, password, email
                    if username == a and password == b:
                        sukses = True
                        pin_required = False
                        break
    ...
    if sukses:
        if not pin_required:
            messagebox.showinfo(title="PIN Required", message="Please set up a PIN for your account.")
            setup_pin(username)
        else:
            messagebox.showinfo(title="Login Success", message="You successfully logged in.")
            window.withdraw()  # Hide the login window
            email_user = username  # Simulate fetching email
            halaman_menu()
    else:
        messagebox.showerror(title="Error", message="Invalid username or password.")

def setup_pin(username):
    pin_window = Toplevel()
    ...
    Label(pin_window, text="Create a 4-digit PIN:", font=("Arial", 12)).pack(pady=10)
    pin_entry = Entry(pin_window, show="*", font=("Arial", 12), width=10)
    pin_entry.pack()
            ...
            try:
                ...
                with open('data/login.csv', 'r') as file:
                    ...
                    for row in reader:
                        if row[0] == username:
                            if len(row) == 3:
                                row.append(pin)  # Add PIN
                            elif len(row) == 4:
                                row[3] = pin  # Update PIN
                        rows.append(row)
                with open('data/login.csv', 'w', newline="") as file:
                    writer = csv.writer(file)
                    writer.writerows(rows)
                messagebox.showinfo(title="Success", message="PIN setup successful.")
                pin_window.destroy()
                halaman_menu()
            except Exception as e:
                messagebox.showerror(title="Error", message=f"An error occurred: {e}")
        else:
            messagebox.showerror(title="Error", message="PIN must be a 4-digit number.")

    Button(pin_window, text="Save PIN", command=save_pin, width=10).pack(pady=10)
...
def signup():
    ...
    if email_form(email):
        try:
            ...
            def verify_otp():
                if otp_entry.get() == kode_otp:
                    try:
                        with open('data/login.csv', 'a', newline="") as file:
                            writer = csv.writer(file)
                            writer.writerow([name, password, email, ""])
                        messagebox.showinfo(title="Success", message="Signup successful. Please log in.")
                        otp_window.destroy()
                    except Exception as e:
            ...
            Button(otp_window, text="Verify", command=verify_otp, width=10).pack(pady=10)
        except Exception as e:
            messagebox.showerror(title="Error", message=f"An error occurred: {e}")
    else:
...
def confirm_payment_makanan():
    ...
    if proceed:
            ...
            with open('data/login.csv', 'r') as file:
                ...
                for row in reader:
                    if len(row) == 4 and row[0] == username:
                        stored_pin = row[3]
                        user_pin = simpledialog.askstring("PIN", "Masukkan PIN Anda:", show='*')
        ...
        if pin_correct:
            struk_beli_makanan(username, email_address, current_time, total, None, cart)
            save_transaction_produk(cart, total, "Non Tunai", current_time, email_address)
            messagebox.showinfo("Sukses", "Pembayaran berhasil, apabila ingin mencetak transaksi silahkan menuju menu printout")
        else:
            messagebox.showerror("Error", "Pembayaran gagal. PIN tidak valid.")
    ...
    def submit():
        ...
        def confirm_payment():
            ...
            if proceed:
                ...
                if pin_correct:
                    struk_konsultasi_grooming(username, email_address, current_time, date, waktu_awal, None)
